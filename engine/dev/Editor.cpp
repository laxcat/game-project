...
void Editor::guiCamera() {
    if (CollapsingHeader("Camera (Orbit Target)")) {
        ...
        SliderFloat("Distance", &mm.camera.distance, 0.0f, 50.0f, "%.3f");
        SliderAngle("Pitch", &mm.camera.pitch, -89.999f, 0.f, "%.3f");
        SliderAngle("Yaw", &mm.camera.yaw);
        SliderAngle("FOV", &mm.camera.fov, 1.f, 89.f);
        SliderFloat3("Target", (float *)&mm.camera.target, -10.0f, 10.0f, "%.3f");
